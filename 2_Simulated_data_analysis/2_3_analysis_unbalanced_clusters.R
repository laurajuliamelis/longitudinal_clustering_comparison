...
# ========  Analysis with simulated data with unbalanced clusters   ========== #
# ============================================================================ #
...
## 1.1. Tres grups ----
prop_grupos <- c(NonResp = 0.30, Fast_Resp = 0.45, Slow_Resp = 0.25)
...
sim_data_3_sep <- add_realistic_missingness(sim_data_3_sep)
...
sim_data_3_sep_sol <- add_realistic_missingness(sim_data_3_sep_sol)
...
sim_data_3_sol <- add_realistic_missingness(sim_data_3_sol)
...
## 1.2. Quatre grups ----
prop_grupos <- c(Severe_NonResp = 0.10, Moderate_NonResp = 0.20, Fast_Resp = 0.45, Slow_Resp = 0.25)
n_grupos <- round(n_ind * prop_grupos)
...
sim_data_4_sep <- add_realistic_missingness(sim_data_4_sep)
...
sim_data_4_sep_sol <- add_realistic_missingness(sim_data_4_sep_sol)
...
sim_data_4_sol <- add_realistic_missingness(sim_data_4_sol)
...
grid.arrange(p_3_sep, p_3_sep_sol, p_3_sol, p_4_sep, p_4_sep_sol, p_4_sol, ncol=3)

# Save plots:
pdf(paste0(path, "/Plots_simulations_unbalanced.pdf"), pointsize = 22, width = 19, height = 10)
...
x <- gridsearch(hlme(PANSS ~ time, random = ~1, subject = "id", data = sim_data_3_sep, 
                     mixture = ~ time, ng = k, B = random(lcmm.linear)), 
                rep = 100, maxiter = 10, minit = lcmm.linear)
sim_data_3_sep_wide <-  merge(sim_data_3_sep_wide, x$pprob[,1:2], by = "id") %>% 
  rename(group.lcmm.lin = class)
x <- gridsearch(hlme(PANSS ~ time + I(time^2), random = ~1, subject = "id", data = sim_data_3_sep,
                     mixture = ~ time + I(time^2), ng = k, B = random(lcmm.quad)), 
                rep = 100, maxiter = 10, minit = lcmm.quad)
sim_data_3_sep_wide <-  merge(sim_data_3_sep_wide, x$pprob[,1:2], by = "id") %>% 
  rename(group.lcmm.quad= class)
x <- gridsearch(hlme(PANSS ~ns(time, knots = c(2, 6)), random = ~1, subject = "id", data = sim_data_3_sep, 
                     mixture = ~ns(time, knots = c(2, 6)), ng = k, B = random(lcmm.splines)), 
                rep = 100, maxiter = 10, minit = lcmm.splines)
sim_data_3_sep_wide <-  merge(sim_data_3_sep_wide, x$pprob[,1:2], by = "id") %>% 
...
x <- gridsearch(hlme(PANSS ~ time, random = ~1, subject = "id", data = sim_data_3_sep_sol, mixture = ~ time, ng = k, B = random(lcmm.linear)), rep = 100, maxiter = 10, minit = lcmm.linear)
sim_data_3_sep_sol_wide <-  merge(sim_data_3_sep_sol_wide, x$pprob[,1:2], by = "id") %>% 
  rename(group.lcmm.lin = class)
x <- gridsearch(hlme(PANSS ~ time + I(time^2), random = ~1, subject = "id", data = sim_data_3_sep_sol, mixture = ~ time + I(time^2), ng = k, B = random(lcmm.quad)), rep = 100, maxiter = 10, minit = lcmm.quad)
sim_data_3_sep_sol_wide <-  merge(sim_data_3_sep_sol_wide, x$pprob[,1:2], by = "id") %>% 
  rename(group.lcmm.quad= class)
x <- gridsearch(hlme(PANSS ~ns(time, knots = c(2, 6)), random = ~1, subject = "id", data = sim_data_3_sep_sol, mixture = ~ns(time, knots = c(2, 6)), ng = k, B = random(lcmm.splines)), rep = 100, maxiter = 10, minit = lcmm.splines)
sim_data_3_sep_sol_wide <-  merge(sim_data_3_sep_sol_wide, x$pprob[,1:2], by = "id") %>% 
  rename(group.lcmm.spl = class)
...
x <- gridsearch(hlme(PANSS ~ time, random = ~1, subject = "id", data = sim_data_3_sol, mixture = ~ time, ng = k, B = random(lcmm.linear)), rep = 100, maxiter = 10, minit = lcmm.linear)
sim_data_3_sol_wide <-  merge(sim_data_3_sol_wide, x$pprob[,1:2], by = "id") %>% 
  rename(group.lcmm.lin = class)
x <- gridsearch(hlme(PANSS ~ time + I(time^2), random = ~1, subject = "id", data = sim_data_3_sol, mixture = ~ time + I(time^2), ng = k, B = random(lcmm.quad)), rep = 100, maxiter = 10, minit = lcmm.quad)
sim_data_3_sol_wide <-  merge(sim_data_3_sol_wide, x$pprob[,1:2], by = "id") %>% 
  rename(group.lcmm.quad= class)
x <- gridsearch(hlme(PANSS ~ns(time, knots = c(2, 6)), random = ~1, subject = "id", data = sim_data_3_sol, mixture = ~ns(time, knots = c(2, 6)), ng = k, B = random(lcmm.splines)), rep = 100, maxiter = 10, minit = lcmm.splines)
sim_data_3_sol_wide <-  merge(sim_data_3_sol_wide, x$pprob[,1:2], by = "id") %>% 
  rename(group.lcmm.spl = class)
...
x <- gridsearch(hlme(PANSS ~ time, random = ~1, subject = "id", data = sim_data_4_sep, mixture = ~ time, ng = k, B = random(lcmm.linear)), rep = 100, maxiter = 10, minit = lcmm.linear)
sim_data_4_sep_wide <-  merge(sim_data_4_sep_wide, x$pprob[,1:2], by = "id") %>% 
  rename(group.lcmm.lin = class)
x <- gridsearch(hlme(PANSS ~ time + I(time^2), random = ~1, subject = "id", data = sim_data_4_sep, mixture = ~ time + I(time^2), ng = k, B = random(lcmm.quad)), rep = 100, maxiter = 10, minit = lcmm.quad)
sim_data_4_sep_wide <-  merge(sim_data_4_sep_wide, x$pprob[,1:2], by = "id") %>% 
  rename(group.lcmm.quad= class)
x <- gridsearch(hlme(PANSS ~ns(time, knots = c(2, 6)), random = ~1, subject = "id", data = sim_data_4_sep, mixture = ~ns(time, knots = c(2, 6)), ng = k, B = random(lcmm.splines)), rep = 100, maxiter = 10, minit = lcmm.splines)
sim_data_4_sep_wide <-  merge(sim_data_4_sep_wide, x$pprob[,1:2], by = "id") %>% 
  rename(group.lcmm.spl = class)
...
x <- gridsearch(hlme(PANSS ~ time, random = ~1, subject = "id", data = sim_data_4_sep_sol, mixture = ~ time, ng = k, B = random(lcmm.linear)), rep = 100, maxiter = 10, minit = lcmm.linear)
sim_data_4_sep_sol_wide <-  merge(sim_data_4_sep_sol_wide, x$pprob[,1:2], by = "id") %>% 
  rename(group.lcmm.lin = class)
x <- gridsearch(hlme(PANSS ~ time + I(time^2), random = ~1, subject = "id", data = sim_data_4_sep_sol, mixture = ~ time + I(time^2), ng = k, B = random(lcmm.quad)), rep = 100, maxiter = 10, minit = lcmm.quad)
sim_data_4_sep_sol_wide <-  merge(sim_data_4_sep_sol_wide, x$pprob[,1:2], by = "id") %>% 
  rename(group.lcmm.quad= class)
x <- gridsearch(hlme(PANSS ~ns(time, knots = c(2, 6)), random = ~1, subject = "id", data = sim_data_4_sep_sol, mixture = ~ns(time, knots = c(2, 6)), ng = k, B = random(lcmm.splines)), rep = 100, maxiter = 10, minit = lcmm.splines)
sim_data_4_sep_sol_wide <-  merge(sim_data_4_sep_sol_wide, x$pprob[,1:2], by = "id") %>% 
  rename(group.lcmm.spl = class)
...
x <- gridsearch(hlme(PANSS ~ time, random = ~1, subject = "id", data = sim_data_4_sol, mixture = ~ time, ng = k, B = random(lcmm.linear)), rep = 100, maxiter = 10, minit = lcmm.linear)
sim_data_4_sol_wide <-  merge(sim_data_4_sol_wide, x$pprob[,1:2], by = "id") %>% 
  rename(group.lcmm.lin = class)
x <- gridsearch(hlme(PANSS ~ time + I(time^2), random = ~1, subject = "id", data = sim_data_4_sol, mixture = ~ time + I(time^2), ng = k, B = random(lcmm.quad)), rep = 100, maxiter = 10, minit = lcmm.quad)
sim_data_4_sol_wide <-  merge(sim_data_4_sol_wide, x$pprob[,1:2], by = "id") %>% 
  rename(group.lcmm.quad= class)
x <- gridsearch(hlme(PANSS ~ns(time, knots = c(2, 6)), random = ~1, subject = "id", data = sim_data_4_sol, mixture = ~ns(time, knots = c(2, 6)), ng = k, B = random(lcmm.splines)), rep = 100, maxiter = 10, minit = lcmm.splines)
sim_data_4_sol_wide <-  merge(sim_data_4_sol_wide, x$pprob[,1:2], by = "id") %>% 
  rename(group.lcmm.spl = class)
...
prop_grupos_3 <- c(NonResp = 0.30, Fast_Resp = 0.45, Slow_Resp = 0.25)
prop_grupos_4 <- c(Severe_NonResp = 0.10, Moderate_NonResp = 0.20, Fast_Resp = 0.45, Slow_Resp = 0.25)
...
save.image("results_sim_unbal_3g_4g_clust.Rdata")
...
save.image("results_sim_unbal_3g_4g_clust_lcmm.Rdata")
